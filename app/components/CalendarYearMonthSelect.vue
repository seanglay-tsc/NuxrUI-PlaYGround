    ...
    <template #content>
      <template v-if="selectedMode === 'month'">
        ...
        <div class="flex items-center justify-between gap-2">
          <UButton
            color="neutral" variant="ghost" icon="lucide:chevron-left" square
            :disabled="!canDecreaseDisplayedYear" @click="changeDisplayedYear('prev')"
          />
          ...
          <UButton
            color="neutral" variant="ghost" icon="lucide:chevron-right" square
            :disabled="!canIncreaseDisplayedYear" @click="changeDisplayedYear('next')"
          />
        </div>
        <div class="grid grid-cols-4 gap-1 pt-2">
          <UButton
            v-for="date in createYear({ dateObj: displayedYear, pagedNavigation: true })" :key="date.toString()"
            color="neutral" active-color="primary" variant="ghost" active-variant="link" block
            :active="isActiveMonth(date)" :disabled="!canSelectMonth(date)"
      ...
      <template v-else>
        ...
        <div class="flex items-center justify-between gap-2">
          <UButton
            color="neutral" variant="ghost" icon="lucide:chevron-left" square
            :disabled="!canDecreaseDisplayedYearRange" @click="changeDisplayedYearRange('prev')"
          />
          <UPopover :ui="{ content: 'w-fit space-y-2' }">
            <UButton color="neutral" variant="ghost" @click="toggleSelectMode()">
          ...
          <UButton
            color="neutral" variant="ghost" icon="lucide:chevron-right" square
            :disabled="!canIncreaseDisplayedYearRange" @click="changeDisplayedYearRange('next')"
          />
        </div>
        <div class="grid grid-cols-4 gap-1 pt-2">
          <UButton
            v-for="date in createDecade({ dateObj: displayedYearRange.lastYear, endIndex: 12 })"
            :key="date.toString()" color="neutral" active-color="primary" variant="ghost" active-variant="link" block
            :active="isActiveYear(date)" :disabled="!canSelectYear(date)" @click="selectYear(date)"
          >
            {{ formatYear(date) }}
          </UButton>
        </div>
